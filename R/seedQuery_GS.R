seedQuery_GS <- function (G, seed, search_r = 2, r = 0.1, maxsize = NULL)
{
  #if (!require(igraph)) {
  ...
  while (TRUE) {
    ...
    for (rad in 1:d) {
      ...
      if (subsum.u > subsum * (1 + r)) {
        tmp <- unlist(lapply(best.nodes[1], function(x) node2treePath(net,
                                                                   V(subG)$name, x)))
    ...
    if (is.null(maxsize)){
      if (length(subx) == vcount(subG))#length(subx) == vcount(subG)
              break
    }
    if (!is.null(maxsize)) {
